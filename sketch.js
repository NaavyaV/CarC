...
function setup() {
  ...
  fuel = new Fuel(((Math.round(random(fuelRandomNum))) * 100)/2, -truckRandomY, canW/10, canH/(34/8), 10, fuelImg);
...
if(isTouching(player, truck4)){
  ...
  drawSprites();
    ...
    fill('yellow')
    textSize(Math.floor(canArea/6000))
    titleText = text('Car Crashers', (canW/3.2)/2, canH/4)
    textSize(Math.floor(canArea/15000))
    msText = text('High Score: ' + metersSaved + 'm', (canW)/3.3, (canH / (7/6)))
    startButton.show()
